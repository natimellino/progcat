...
ProofIdList : {X : Obj Sets} → (xs : List X) → mapList (iden Sets) xs ≅ iden Sets xs
ProofIdList [] = refl
ProofIdList (x ∷ xs) = cong (λ x₁ → x ∷ x₁) (ProofIdList xs)


open Cat Sets using () renaming (_∙_ to _∙s_)

ProofCompList : {X Y Z : Obj Sets} {f : Hom Sets Y Z} {g : Hom Sets X Y} →
      (xs : List X) → mapList (f ∙s g) xs ≅ (mapList f ∙s mapList g) xs
ProofCompList  [] = refl
ProofCompList  {f = f} {g = g} (x ∷ xs) = cong (λ x₁ → (f (g x)) ∷ x₁) (ProofCompList xs) 

ListF : Fun Sets Sets
ListF = functor List
            mapList
            (ext (λ x → ProofIdList x))
            (ext ProofCompList)
